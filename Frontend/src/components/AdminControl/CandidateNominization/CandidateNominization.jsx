...
const CandidateNominization = ({ state }) => {
        ...
        if (data.result === 1) {
          try {
            const response = await fetch(
              "http://localhost:4000/isCandidateNominated",
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
            ...
            if (response.ok) {
              if (data.result === 0) {
                try {
                  const response = await fetch(
                    "http://localhost:4000/CrimeRecordExist",
                    {
                      method: "POST",
                      headers: {
                        "Content-Type": "application/json",
                  ...
                  if (response.ok) {
                    if (data.result === 0) {
                      ...
                      setLoading(false);
                      setShowPopup(true);
                      setPopupMessage0("Voter ID has Crime Record.");
                ...
                setLoading(false);
                setShowPopup(true);
                setPopupMessage0("Voter ID has already Nominated.");
                setPopupMessage1("Please enter a different Voter ID.");
              }
            } else {
              setLoading(false);
  ...
  const fetchVoterDetails = async (voterId) => {
    try {
      ...
      if (response.ok) {
        ...
        setmobileNo("");
        setProperty("");
        setName(data.voter.name);
        setAge(data.voter.age);
        setGender(data.voter.gender);
        setCitizen(data.voter.citizen);
        setmobileNo(data.voter.mobileNumber);
      } else {
        clearForm();
        setShowPopup(true);
        setPopupMessage0("Please Enter Valid Voter Id");
        setPopupMessage1("");
        console.error(data.error);
      }
    } catch (error) {
  ...
  const handleContractMethodCall = async () => {
    ...
    try {
      ...
      if (age < 25) {
        setLoading(false);
        setShowPopup(true);
        setPopupMessage0("Age must be 25 or above.");
        setPopupMessage1("");
        return;
      }

      if (voterId && name && age && gender && citizen && mobileNo && property) {
        await contract.methods
          .nominateCandidate(voterId, property)
          .send({ from: account });
        setLoading(false);
        clearForm();
  ...
  return (
    <>
      <div className="min-w-screen flex items-center justify-center px-2 py-2 mt-4 ml-32">
        <div
                ...
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="mt-2 text-lg font-medium leading-6 text-gray-900">
                      Candidate Nominate Register Successfully
                    </h3>
                  </div>
                </div>
              </div>
...
export default CandidateNominization;
