...
const VoterReg = ({ state }) => {
  ...
  const handleVerify = async () => {
    try {
      const response = await fetch(
        "http://localhost:4000/checkReferenceIdUsed",
        {
          method: "POST",
          headers: {
      ...
      if (response.ok) {
        if (data.result === 1) {
          fetchVoterDetails(referenceId);
        } else if (data.result === 0) {
          setShowPopup(true);
          setPopupMessage0("Reference ID already used.");
          setPopupMessage1("Please enter a different Reference ID.");
        }
      } else {
  ...
  const fetchVoterDetails = async (referenceId) => {
    try {
      const response = await fetch("http://localhost:4000/getRegVoterDataset", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
      ...
      if (response.ok) {
        ...
        setAge("");
        setShowPopup(true);
        setPopupMessage0("Please Enter Valid Reference ID");
        setPopupMessage1("");
        console.error(data.error);
      }
    } catch (error) {
  ...
  const handleContractMethodCall = async () => {
    setLoading(true);
    try {
      if (!referenceId || !name || !age || !gender || !citizen || !mobileNo) {
        setLoading(false);
        setShowPopup(true);
        setPopupMessage0("Please fill in all required fields.");
        setPopupMessage1("");
        return;
      }
      if (mobileNo.length !== 10) {
        setLoading(false);
        setShowPopup(true);
        setPopupMessage0("Mobile number must be exactly 10 digits.");
        setPopupMessage1("");
        return;
      }
      ...
      if (referenceId && name && age && gender && citizen && mobileNo) {
        await contract.methods
          .regVoter(referenceId, gender, citizen, mobileNo)
          .send({ from: account });
        const RegisteredVoterId = await contract.methods
          .getLatestRegisteredVoterId()
          .call();
        setLoading(false);
        clearForm();
        setSuccessPopup(true);
        setSuccessPopupMessage(RegisteredVoterId);
      } else {
        setLoading(false);
        console.error("Some data is missing or not verified.");
      }
    } catch (error) {
      setLoading(false);
  ...
  return (
    <>
      <div className="min-w-screen flex items-center justify-center px-2 py-2 mt-4">
        <div
            ...
            <div className="w-full md:w-1/2 py-10 px-5 md:px-10">
              ...
              <div>
                <div className="flex -mx-3">
                  ...
                  <div className="w-1/2 px-3 py-3 mb-1">
                    <button
                      className="m-4 inline-flex items-center justify-center rounded-xl border bg-white px-5 py-3 font-medium text-blue-600 shadow hover:bg-blue-50 h-8"
                ...
                <div className="flex -mx-3">
                  <div className="w-1/2 px-3 mb-5">
                    ...
                    <div className="flex">
                      <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center">
                        <i className="mdi mdi-gender-male-female text-gray-400 text-lg" />
                      </div>
                      <select
                        value={gender}
                ...
                <div className="flex -mx-3">
                  <div className="w-1/2 px-3 mb-5">
                    ...
                    <div className="flex">
                      <div className="w-10 z-10 pl-1 text-center pointer-events-none flex items-center justify-center">
                        <i className="mdi mdi-earth text-gray-400 text-lg" />
                      </div>
                      <select
                        value={citizen}
                ...
                <div className="flex -mx-3">
                  <div className="w-full px-3 mb-5">
                    <button
                      ...
                      onClick={handleContractMethodCall}
                      disabled={loading}
                    >
                ...
                <div className="sm:flex sm:items-start">
                  ...
                  <div className="mt-3 text-center sm:mt-0 sm:ml-4 sm:text-left">
                    <h3 className="text-lg font-medium leading-6 text-gray-900">
                      Voter Register Successfully
                    </h3>
                    <div className="mt-2">
                      <p className="text-sm text-gray-500">
                        Voter ID : {successPopupMessage}
                      </p>
                    </div>
                  </div>
                </div>
